...
function MinimalOverlay({ open, onClose, children }: { open: boolean; onClose: () => void; children: React.ReactNode }) {
  ...
  return (
    ...
    >
      <div
        ref={overlayRef}
        className="relative w-full max-w-6xl mx-auto bg-white/90 rounded-xl border border-white/30 shadow-lg flex flex-col items-center min-h-[50vh] max-h-[95vh] min-w-[320px] p-0"
...
function AnalyticsOverlay() {
  ...
  function ErrorsTab() {
    ...
    return (
      <div className="w-full flex flex-col items-center gap-8">
        <div className="w-full max-w-md mx-auto bg-white/80 rounded-xl shadow p-6 border border-gray-100">
          <h2 className="text-xl font-semibold text-gray-800 mb-4 flex items-center gap-2"><AlertTriangle className="w-5 h-5 text-yellow-500" /> Error Breakdown</h2>
          {errorTypesArr.length > 0 ? (
            ...
            <div className="text-gray-400 text-base text-center">No errors in this test!</div>
          )}
        </div>
      </div>
    );
  ...
  return (
    <MinimalOverlay open={open === 'analytics'} onClose={closeOverlay}>
      {/* Tab Bar */}
      <nav className="w-full flex flex-row justify-center items-center gap-2 border-b border-gray-100 bg-white/80 rounded-t-xl sticky top-0 z-10" style={{backdropFilter:'blur(8px)',WebkitBackdropFilter:'blur(8px)'}}>
        {TABS.map(t => (
          <button
            key={t.key}
            className={`flex items-center gap-2 px-6 py-3 text-base font-semibold rounded-t-xl transition-all duration-200 focus:outline-none ${tab === t.key ? 'bg-white shadow text-blue-700' : 'text-gray-500 hover:text-blue-700 hover:bg-gray-50'}`}
            onClick={() => setTab(t.key)}
            style={{borderBottom: tab === t.key ? '2.5px solid #3b82f6' : '2.5px solid transparent'}}
          >
